local acs_index,events,ver = nil,nil,nil
local function new()
...
local function bypass1()
	...
	warn('bypass started')
	warn('waiting...')
	game:GetService('ReplicatedStorage').ACS_Engine.Events.Refil:FireServer(game.Players.LocalPlayer.Character.ACS_Client.Kit.Fortifications, -99999999999)
	warn('bypassed successfully!')
    warn('--------------------------------------------')
end
...
local function bypass2()
	warn('--------------------------------------------')
	warn('bypass started')
	warn('waiting...')
    game:GetService('ReplicatedStorage')['ACS_Engine'].Eventos.Recarregar:FireServer(999999999999999,{ACS_Modulo={Variaveis={StoredAmmo = game.Players.LocalPlayer.Character.Saude.Kit.Fortifications}}})
    warn('bypassed successfully!')
    warn('--------------------------------------------')
end
s.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui")
...
for num,child in pairs(game.ReplicatedStorage:GetDescendants()) do
	...
	if child.Name == 'Events' and child.Parent == acs_index then
		events = child
		ver = '2.0.1'
		print('Version: '..ver)
	end
